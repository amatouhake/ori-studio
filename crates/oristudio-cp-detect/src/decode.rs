...
mod tests {
    ...
    /// Zero-crease-evidence input must not decode to status "valid".
    ///
    ...
    fn exact_solve_refuses_zero_evidence_border_only_output() {
        ...
        // no contacts — the synthetic zero-evidence input from the
        // cross-backend decode differential probe (`blank()`).
        let size = 64usize;
        let pixels = size * size;
