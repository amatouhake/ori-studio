...
mod tests {
    ...
    fn beam_rejects_zero_length_span_despite_high_support() {
        // A zero-length span (coincident vertices, zero t_interval)
        // with high line support must not displace normal spans. Control
        // {normal, normal} selects both, while the zero-length decoy (id 1)
        // displaced span 0 and selected {1} alone.
        let points = vec![
            Point2::new(0.0, 0.0),
            Point2::new(0.5, 0.0),
            Point2::new(1.0, 0.0),
