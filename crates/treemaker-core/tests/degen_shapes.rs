//! Degenerate-shape regression tests: empty paths, mismatched rings, empty polygon paths.
//!
...
/// too (the "cleared" variant, which reaches a different walk).
...
#[test]
fn empty_path_text_is_rejected_at_parse() {
...
#[test]
fn empty_path_nodes_make_optimizers_err_never_panic() {
...
// ---- ring_nodes/ring_paths mismatch ----

#[test]
fn truncated_ring_paths_kept_text_is_rejected_at_parse() {
...
#[test]
fn truncated_ring_paths_cleared_text_is_rejected_at_parse() {
    let (tree, pos) = honestly_built_polys();
...
#[test]
fn cleared_ring_paths_in_memory_make_build_err_never_panic() {
    ...
    match outcome {
        ...
        Err(_) => panic!("build panicked on mismatched rings"),
    }
}

// ---- empty path in the polygon ring walk ----

#[test]
fn empty_polygon_path_makes_poly_build_err_never_panic() {
    let mut tree = Tree::from_tmd_str(SEED_V4).expect("seed parses");
    assert!(
    ...
    match outcome {
        ...
        Err(_) => panic!("build panicked on an empty polygon path"),
    }
}
