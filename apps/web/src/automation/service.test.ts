...
describe('semantic MCP contracts', () => {
  ...
  it('selects the native importer geometry, including frame-only files', () => {
    ...
    expect(guard({ file_frames: [{ ...good, frame_classes: ['foldedForm'] }, good] })).not.toThrow();
    expect(guard({ ...good, file_frames: [{ ...good, vertices_coords: [] }] })).not.toThrow();
    expect(guard({ file_frames: [{ vertices_coords: [[0, 0], [1, 1], [0, -100]], edges_vertices: [[0, 1]] }] })).not.toThrow();
  });
  it('passes negative-y and single-axis FOLD through to the importer', () => {
    // The importer normalizes both (#366, #367); the guard only rejects structurally unusable files.
    expect(() => engines.guardFoldImport('{"edges_vertices":[[0,1]],"vertices_coords":[[0,0],[1,0]]}')).not.toThrow();
    expect(() => engines.guardFoldImport('{"edges_vertices":[[0,1]],"vertices_coords":[[0,-2],[1,-1]]}')).not.toThrow();
    expect(() => engines.guardFoldImport('{"edges_vertices":[[0,1]],"vertices_coords":[[0,0],[1,1]]}')).not.toThrow();
  });
  it('refuses structurally unusable FOLD before import', () => {
    expect(() => engines.guardFoldImport('{"edges_vertices":[[0,1]],"vertices_coords":[[0,0]]}')).toThrow('valid vertices');
    expect(() => engines.guardFoldImport('{"edges_vertices":[],"vertices_coords":[[0,0],[1,1]]}')).toThrow('finite vertices');
    expect(() => engines.guardFoldImport('{"edges_vertices":[[0,1]],"vertices_coords":[[0,0],[1,null]]}')).toThrow('finite vertices');
  });
});
