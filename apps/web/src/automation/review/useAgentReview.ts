...
export function useAgentReview() {
  ...
  const [preview, setPreview] = useState<{ images: string[]; revision: number; draftId: string; checkpoint: string; view: string } | null>(null);
  ...
  const apply = (takeOver: boolean, includeSource: boolean) => action(async () => {
    ...
    if (currentCheckpoint || (chosenView === 'pose' && jobId && !job?.adopted)) {
      const forked = await mutate('fork_design', { title: d.title, ...(currentCheckpoint ? { checkpoint_id: currentCheckpoint } : { job_id: jobId }) });
      target = { draft_id: String(forked.draft_id), revision: Number(forked.revision) };
      setSelected(target.draft_id); setCheckpoint('');
    }
    if (takeOver && target.draft_id !== d.draft_id) service.takeOver(target.draft_id, target.revision);
    value(await service.call('commit_design', { ...target, request_id: crypto.randomUUID(), label: t('common:agentReview.applyLabel', 'Apply agent proposal'), include_source: includeSource }, 'human'));
  ...
  const save = () => action(async () => {
    ...
    // retained variant first so they also remain continuable in the review.
  ...
  const shown = !!preview && preview.draftId === d?.draft_id && preview.checkpoint === currentCheckpoint && preview.revision === (checkpointRevision ?? d?.revision) && preview.view === chosenView;
  return { t, open, setOpen: setAgentReviewOpen, drafts: snapshot.drafts, d, checkpoint: currentCheckpoint,
    ...
    checkpointNow: () => action(async () => { await mutate('checkpoint_design', { label: t('common:agentReview.stepLabel', 'Review step {{revision}}', { revision: d?.revision }) }); }, 'checkpoint'),
