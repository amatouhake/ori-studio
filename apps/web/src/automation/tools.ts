...
const brief = object({
  goal: str(), constraints: array(str(), 24), preferences: array(str(), 24),
  delegated_decisions: array(str(), 24), delivery_stage: choice('exploration', 'base', 'shaped_model'),
  paper: object({ shape: choice('square', 'rectangle', 'unrestricted'), sheets: integer(1, 64) }),
}, ['goal']);
...
export const TOOLS = [
  ...
  tool('begin_design', 'Begin an isolated experiment. source=active clones the Edit canvas or the active design tab of the requested kind; source=new creates a square CP or empty TreeMaker/BP tree. For BP, initialize_tree seeds the first root and leaves. source=import reads supplied content, never a host path. Existing user work is preserved.', {
    ...
    brief, format: choice('cp', 'fold', 'ori', 'tmd5', 'bps'), content: { type: 'string', maxLength: LIMITS.inputBytes },
  }, ['request_id', 'source', 'kind']),
  tool('fork_design', 'Continue a proposal or checkpoint as a separate isolated variant. A completed layout-search candidate or static-pose job can be adopted explicitly. The brief and captured source relationship are inherited; the parent stays unchanged.', { ...mutation, title: str(), from_source: { type: 'boolean' }, checkpoint_id: str(), job_id: str(), candidate_index: integer(0, 3) }, [...Object.keys(mutation), 'title']),
  tool('retain_design', 'Keep a proposal for this access session (exempt from idle expiry), or release that retention. Export before disabling access or restarting. This does not grant publication permission.', { ...mutation, keep: { type: 'boolean' } }),
  ...
  tool('checkpoint_design', 'Save a named recovery point, including the adopted static placement, inside an experiment without touching application history.', { ...mutation, label: str() }),
  tool('rollback_design', 'Restore a checkpoint and its saved static placement atomically; increments revision and invalidates old analysis without losing the saved pose. A stale revision is refused.', { ...mutation, checkpoint_id: str() }),
  tool('analyze_design', 'Start a bounded asynchronous analysis job. CP checks return localized Oriedita diagnostics; flat_fold runs actual layer-order solving. TreeMaker optimize/build modifies only the experiment on success; layout_search returns seeded alternatives without changing it (adopt with fork_design). BP packing returns real kernel validation. paper audits source sheet dimensions or CP boundary scope against the optional brief. None measures appearance or proves motion. Poll job_status.', { ...mutation, analysis: choice('checks', 'flat_fold', 'packing', 'optimize_scale', 'optimize_edges', 'optimize_strain', 'build_cp', 'paper', 'layout_search'), trials: integer(1, 16), keep: integer(1, 4), seed: integer(0, 4294967295), starting_face: integer(1), case_limit: integer(1, 16) }, [...Object.keys(mutation), 'analysis']),
  tool('pose_design', 'Compute a static 3D pose with the existing 3D folding kernel. Optional angle targets apply only to a private copy of existing creases; no creases are added and the draft stays unchanged. A placed pose is not a folding motion or simulation success. Render view=pose, or fork from the job to adopt the angle targets.', { ...mutation, angles: array(object({ line_id: integer(1, LIMITS.lines), assignment: choice('mountain', 'valley'), angle: num(0, 180) }), 128), starting_face: integer(1) }, Object.keys(mutation)),
  ...
  tool('render_view', 'Return PNG images and revision-bound metadata for design geometry, CP, flat-fold result, simulation or static pose. Evaluation views omit diagnostic labels. Diagnostic CP views expose exact source anchors; pose views link projected face bounds to source CP line IDs. Bounds use a 1024-square viewBox and are not pixel visibility masks. Pose/simulation accept up to four cameras. Use checkpoint_id for a frozen step, including its saved pose. Pose views without job_id use the adopted placement. Explicit job artifacts require their matching revision.', { ...draft, view: choice('crease_pattern', 'folded', 'simulation', 'pose', 'design'), purpose: choice('evaluation', 'diagnostic'), cameras: array(choice('isometric', 'top', 'front', 'side'), 4, 1), line_ids: ids, checkpoint_id: str(), job_id: str(), camera: choice('isometric', 'top', 'front', 'side'), size: integer(128, 2048) }, [...Object.keys(draft), 'view'], true),
  ...
  tool('commit_design', 'Publish the experiment into the application. CP replacement is ONE undoable action and requires the live revision token captured at begin_design; concurrent user edits cause conflict. New CP experiments also capture a live token, so they cannot overwrite later work. The experiment remains available for export. Trees publish as a new design tab. Optional job_id pins a placed pose whose angles were already adopted into the CP.', { ...mutation, job_id: str(), label: str('Human-readable undo label.'), include_source: { type: 'boolean', description: 'For a derived CP, atomically publish its captured source as a new design tab together with the CP.' } }, [...Object.keys(mutation), 'label']),
...
export const CAPABILITIES = {
  design_loop: { version: 1, brief: 'Inherited by forks and derivations. Paper is a checked contract; free-text constraints require review.', ownership: 'Human takeover revokes writes to that draft, not the external process.', publication: 'include_source publishes a derived CP and captured source atomically.', views: ['evaluation', 'diagnostic'], layout_search: 'Bounded seeded trials, no mutation; fork_design adopts a candidate.' },
