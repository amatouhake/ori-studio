...
export const TOOLS = [
  ...
  tool('simulate_design', 'Start real Origami Simulator physics in an isolated worker. Fold target is 0..1; explicit max_steps bounds work. Results separate solver_settled from target_attainment (oriented mesh angle residuals, tolerance 5 degrees). Convergence alone does not mean the requested target was attained; inspect outcome and residuals. Not a collision or foldability proof. Poll job_status, then render_view with the job ID.', { ...mutation, fold_amount: num(0, 1), max_steps: integer(1, 20000) }),
