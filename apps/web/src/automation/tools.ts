...
export const CONSTRUCTION_INPUTS = {
  bases: 'blintz/fish_base/dove_base/bird_base/frog_base: two placement anchors, matching the Oriedita template markers; generated lines use assignment (default mountain), so inspect and assign folds afterward.',
...
export const TOOLS = [
  ...
  tool('workspace_history', 'Undo or redo exactly one Edit-canvas action using the application history. Requires the live revision and load serial from workspace; never falls back to another design. Undoing a committed experiment restores its complete prior canvas state.', { request_id: mutation.request_id, live_revision: integer(), load_serial: integer(), direction: choice('undo', 'redo') }),
  ...
  tool('simulate_design', 'Start real Origami Simulator physics in an isolated worker. Fold target is 0..1; explicit max_steps bounds work. Results report convergence, strain and diagnostics (not a proof of foldability). Poll job_status, then render_view with the job ID.', { ...mutation, fold_amount: num(0, 1), max_steps: integer(1, 20000) }),
  ...
  tool('export_design', 'Return editable file/artifact content to the client; no host filesystem access. OSF preserves Ori Studio document data, CP/ORI/FOLD are interchange formats; TMD5/BPS preserve design trees. SVG/PNG and simulation OBJ export actual rendered/solved geometry. Save returned content using your client.', { ...draft, format: choice('osf', 'cp', 'ori', 'fold', 'tmd5', 'bps', 'svg', 'png', 'obj'), job_id: str() }, [...Object.keys(draft), 'format'], true),
