...
Every behavioural statement is tagged with exactly one of:

| Tag | Meaning | What it licenses an agent to do |
| --- | --- | --- |
| **[T]** theorem | A mathematical statement with a primary-source proof or statement. | Rely on it unconditionally, within its hypotheses. |
...
2. **[T] TreeMaker's universal-molecule crease pattern satisfies Kawasaki by
   ...
   Theorem by design"]. **No such guarantee exists for Box Pleating Studio's
   exported CP** — its manual says CP export "is not intended to generate
   flat-foldable CPs" [BPS-manual]. A CP derived from BP must be validated
   like any hand-drawn CP (§3, §4).
...
| **[T] TreeMaker output.** "Our crease pattern satisfies Kawasaki's Theorem by design, and Maekawa's Theorem follows automatically if Justin's conditions are satisfied"; for uniaxial bases the ordering/assignment problem "is not NP-complete". | [LD06 §3.1, §2.2] |
...
result.** At the reported `point`, every incident crease must be (a) assigned
(`assignment` ≠ `unassigned`) and (b) classic (`fold_angle_degrees` = 180).
Otherwise the vertex was judged by the spatial branch and the entry's `kind`
starts with `Spatial`. Both are visible in `inspect_design`.
...
| Fact | Tag | Source |
| --- | --- | --- |
| Oriedita requires local flat-foldability first: "Crease pattern must be flat foldable before folding calculation". | [U] | `help.properties` `check3` |
| `starting_face` (1-based) is the face the estimator starts face-position propagation from; default 1. | [I] | `folding.rs:56-64`, `:1452-1454`; `analysis.ts:60,70` |
...
| Flat-Folder's own vertex check flags Maekawa or Kawasaki violations (Kawasaki tolerance 1e-5 on the even-angle sum − π); import merges vertices closer than `L/300`. | [I] (upstream) | [Flat-Folder README] |
...
| Pinned | [T]/[U] | A node is pinned if it cannot move without violating a path or leaving the paper; an edge if it cannot lengthen. Creases cannot be built while an unpinned leaf node lies inside a polygon. | [TM-help:tutorial_3] |
...
| `add_edge`, `delete_node`, `delete_edge`, `update_node_label`, `move_node` | Edit tree | [U] Deleting cannot split the tree. Moving a branch node changes nothing in the CP. |
...
| `set_symmetry {has_symmetry, sym_loc, sym_angle}` | Tree Inspector "Symmetry" (book = point (0.5,0.5), 90°; diag = 45°) | [U] Symmetry conditions are inert without a symmetry line. |
| `add_condition {kind}` / `update_condition` / `delete_condition` | Condition menu | Kinds `node_on_corner`, `node_on_edge`, `node_symmetric`, `nodes_paired`, `nodes_collinear`, `edge_length_fixed`, `edges_same_strain`, `node_fixed`, `path_active`, `path_angle_fixed`, `path_angle_quant` — direct ports of `tmCondition*::CalcFeasibility()` (`PORTING.md`). [U] Conditions on branch nodes are ignored; avoid redundant conditions [TM-help:tutorial_3]. |
...
| Stretches / GOPS | [I] (upstream) | Valid junctions are grouped into `Stretch`es; for each, a `Repository` searches `Configuration`s → `Device`s per `Partition` → a `Positioner` assembles a `Pattern`. "The algorithm for this has not yet been published in complete details, but an outline of it can be found in our paper" [LT18]. | `src/core/design/layout/README.md` |
| Stretch selection in the UI | [I] (upstream) | The Stretch panel shows two index steppers, "Configuration" and "Pattern", over `repo.configurations[]` and `configurations[index].patterns[]`; "Only one pattern is found." when both counts are 1. No selection criterion is documented anywhere in the vendored source or manual. | `src/app/vue/panel/stretch.vue`; `locale/en.json` `panel.repo.*` |
| Pattern not found | [U] | "It is not always possible for BP Studio to find working stretch patterns in every valid layout." UI message: "Some of the overlappings of flaps in this design are valid, but for now BP Studio is unable to find working stretch patterns for them." | [BPS-manual] "Current limitations"; `locale/en.json` `message.patternNotFound` |
| **CP export is not a flat-foldability guarantee** | [U] | "Note that CP exporting is not intended to generate flat-foldable CPs (which is beyond the scope of BP Studio for now)." | [BPS-manual] |
| Other limits | [U] | "BP Studio hasn't implemented the notion of elevation, half-integral unit structures, or meandering rivers." | [BPS-manual] "Current limitations" |
| Contours | [I] (upstream) | Hinge contours are traced from rough (AABB-union) contours plus pattern contours; invalid layouts are still rendered "as much as possible". | `layout/README.md` |
...
| `build_cp` = `has_full_cp` | A universal-molecule CP with rooted assignment exists; it satisfies Kawasaki by design [LD06 §3.1]. | That the *derived* CP survives Oriedita's tolerances after the FOLD round-trip (§1.1 normalization; run `checks`). | [T]+[I] |
| BP `packing.valid` | All flap-pair distance constraints hold. | That stretches were found for every junction (`layout.patternNotFound`); **that the exported CP is flat-foldable — the upstream disclaims this** [BPS-manual]. | [I]/[U] |
| `checks` → `no_local_issues` | Zero entries of *any* severity from Check1, Check2, Check3 and CheckCamv. **This is a classical local flat-foldability verdict (Kawasaki–Justin, Maekawa–Justin, and the crimp reduction at every interior vertex; no overlaps; no unsplit T-junctions) exactly when the document contains no `None` crease and no non-classic crease** — both checkable in `inspect_design` (`assignment` ≠ `unassigned`, `fold_angle_degrees` = 180 for every line). With `None` creases present, interior vertices touching them were judged by the spatial closure solver and reported (as `info`) or, if only boundary vertices touch them, not judged at all; with non-classic creases present, those vertices were judged by closure residual, not by the theorems. Under the stated precondition, [ADK24 §2] then gives: an isometric flat folding exists. | Global flat-foldability / a layer order [BH96], [ADK24 §3]. | [I]+[T] |
...
| U3 | **Kawasaki survives the FOLD round-trip.** [LD06] guarantees Kawasaki for the TreeMaker CP in TreeMaker's coordinates; the derived CP passes through FOLD export and the §1.1 normalization (uniform scale, so angles are preserved in exact arithmetic) and Oriedita's 1e-6° bar. Not measured. | Run `checks` on a corpus of `derive_crease_pattern` outputs and record the residuals. |
...
| U7 | **`angular_flat_foldability` candidate semantics.** The kernel's candidate list and "commit style" colour rule were read from call sites (`lib.rs:2911-2923`), not from the candidate generator itself. | Read `operations/construction.rs` `vertex_completion_candidates` and the corresponding Oriedita `MouseHandlerFoldableLineDraw`. |
| U8 | **`starting_face` semantics** beyond "propagation start" (whether it fixes orientation/position of the folded figure). | Read Oriedita's fold-start handler and `FoldedFigure` placement code. |
