...
| G16 | TreeMaker-derived CPs fail `CheckCamv` `Angles` at the 1e-6° bar with residuals of order 1e-6°–1e-4° (measured, §4), and `repair: snap` cannot help because TreeMaker angles are not on the 22.5° or box-pleat families. The kernel's own doc notes the bar "rejects ~42%" of real-pattern vertices "which is the status quo … those same patterns fail CAMV in Oriedita today" (`lib.rs:3557-3563`). | §4 measurement; `lib.rs:3557-3574` | An `Angles` entry on a TreeMaker-derived CP with a residual of that order is numerical, not a design error; the agent has no tool to remove it and must report it as such rather than "repair" it by moving vertices. Whether `flat_fold` can proceed on such a CP is open (U3). |
...
| U3 | **Partially resolved (§4, G16).** Measured on two fixtures: TreeMaker CPs do *not* pass the 1e-6° CAMV bar (residuals 1.7e-6°–4.8e-5°), and the editor's fold estimate returned `NotAttempted` / `Contradiction` on them. **Still open:** whether those fold outcomes are caused by the angular residuals, by the exported rooted assignment, or by the estimator's own tolerances; and whether the MCP's `flat_fold` on a `derive_crease_pattern` result behaves the same as the `fold_check` example used here. | Diagnose one fixture in the editor (which stage stops, which faces contradict); a corpus-scale run is not needed for the recipes, which must simply not assume `no_local_issues` after `derive_crease_pattern`. |
